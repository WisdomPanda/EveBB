...
else if (isset($_POST['delete_user']) || isset($_POST['delete_user_comply']))
{
	...
	if (isset($_POST['delete_user_comply']))
	{
		...
		//Delete their api data, keep the character data.
		remove_api_keys($id);
		...
		//Let plugins do their own thing.
		foreach ($_HOOKS['users'] as $hook) {
			$hook->user_deleted($id);
		} //End foreach().
...
// New PMS
	...
	if ($pun_config['o_avatars'] == '1')
	{
		if ($pun_config['o_eve_use_iga'] == '1') {
			$avatar_field = '<img src="img/chars/'.$char['character_id'].'_64.jpg" width="64px" height="64px" alt="" />';
		} else {
	...
	$page_title = array(pun_htmlspecialchars($pun_config['o_board_title']), sprintf($lang_profile['Users profile'], (($pun_config['o_eve_use_iga']) == '1' ? $char['character_name'] : pun_htmlspecialchars($user['username']))));
...
<?php
	...
	{
		$char = fetch_selected_character($id, true);
		...
		if ($pun_config['o_eve_use_iga'] == '1') {
			$user_avatar = '<img src="img/chars/'.$char['character_id'].'_64.jpg" width="64px" height="64px" alt="" />';
		} else {
...
<?php
		...
		define('FROM_PROFILE', true);
		require PUN_ROOT."profile_characters.php";
