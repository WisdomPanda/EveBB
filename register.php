...
if (isset($_POST['form_sent']))
{
	...
	if (!check_alpha_numeric($api_key)) {
		$errors[] = $lang_prof_reg['Bad API Key'];
	} //End if.
	
	if (!check_numeric($api_user_id)) {
		$errors[] = $lang_prof_reg['Bad User ID'];
	} //End if.
	
	if (!check_numeric($api_character_id)) {
		$errors[] = $lang_prof_reg['Bad Character ID'];
	} //End if.
	
	//Now we hit the DB and see if there is already an API auth associated.
	if (empty($errors)) {
		$result = $db->query('SELECT user_id FROM '.$db->prefix.'api_auth WHERE api_character_id='.$api_character_id.' OR api_key=\''.$api_key.'\' OR api_user_id='.$api_user_id.';') or error('Unable to fetch user api info from the database.', __FILE__, __LINE__, $db->error());
		if ($db->num_rows($result) > 0) {
			$errors[] = $lang_prof_reg['Dupe API'];
			//error($lang_prof_reg['Dupe API']);
		} //End if.
	} //End if.
	...
	if (empty($errors))
	{
		...
		//Since we use their character name for registration, we need to display their names.
...
$required_fields = array(
								...
								'req_email1' => $lang_common['Email'],
								'req_email2' => $lang_common['Email'].' 2',
								'api_key' => $lang_common['api_key'],
								'api_user_id' => $lang_common['api_user_id'],
								'api_character_id' => $lang_common['api_character_id']
							);
							
if ($pun_config['o_regs_verify'] == '1') {
	unset($required_fields['req_password1']);
	unset($required_fields['req_password2']);
} //End if.
$focus_element = array('register', 'req_email1');
define('PUN_ACTIVE_PAGE', 'register');
require PUN_ROOT.'header.php';
...
<?php endif; ?>					</div>
			...
			<div class="inform">
				<fieldset>
					<legend><?php echo $lang_register['api_legend'] ?></legend>
					<div class="infldset">
						<label class="required"><strong><?php echo $lang_register['api_user_id'] ?> <span><?php echo $lang_common['Required'] ?></span></strong><br /><input type="text" name="api_user_id" id="api_user_id" value="<?php if (isset($_POST['api_user_id'])) echo pun_htmlspecialchars($_POST['api_user_id']); ?>" size="16" /><br /></label>
						<label class="required"><strong><?php echo $lang_register['api_key'] ?> <span><?php echo $lang_common['Required'] ?></span></strong><br /><input type="text" name="api_key" id="api_key" value="<?php if (isset($_POST['api_key'])) echo pun_htmlspecialchars($_POST['api_key']); ?>" size="16" /><br /></label>
						<label class="required"><strong><?php echo $lang_register['api_character_id'] ?> <span><?php echo $lang_common['Required'] ?></span></strong><br />
						<span id="api_holder"><a class="fetch_chars" href="index.php" onClick="fetchCharacters(); return false;"><span id="char_fetch_text"><?php echo $lang_register['char_fetch_text']; ?></span></a></span>
						</label>
						<p class="clearb"><?php echo $lang_register['api_info'] ?></p>
					</div>
				</fieldset>
			</div>
			<div class="inform">
