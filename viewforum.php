...
if (!$pun_user['is_guest']) {
	if ($pun_user['g_id'] != PUN_ADMIN) {
		$result = $db->query('SELECT DISTINCT f.forum_name, f.redirect_url, f.moderators, f.num_topics, f.sort_by, fp.post_topics, s.user_id AS is_subscribed FROM '.$db->prefix.'forums AS f LEFT JOIN '.$db->prefix.'forum_subscriptions AS s ON (f.id=s.forum_id AND s.user_id='.$pun_user['id'].') INNER JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND (fp.group_id='.$pun_user['g_id'].' '.$group_list.')) LEFT JOIN '.$db->prefix.'forums AS pf ON f.parent_forum_id=pf.id WHERE fp.read_forum=1 AND f.id='.$id) or error('Unable to fetch forum info', __FILE__, __LINE__, $db->error());
	} else {
 	 ...
 	 $result = $db->query('SELECT DISTINCT f.forum_name, f.redirect_url, f.moderators, f.num_topics, f.sort_by, fp.post_topics, 0 AS is_subscribed FROM '.$db->prefix.'forums AS f INNER JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND (fp.group_id='.$pun_user['g_id'].' )) LEFT JOIN '.$db->prefix.'forums AS pf ON f.parent_forum_id=pf.id WHERE fp.read_forum=1 AND f.id='.$id) or error('Unable to fetch forum info', __FILE__, __LINE__, $db->error());
} //End if.
