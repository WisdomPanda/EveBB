...
while ($cur_post = $db->fetch_assoc($result))
{
	...
	//Start ticker generation.
	//I won't move this out of here since we already have all the data we need, without having to fetch more.
	//We'll build the ticker much like eve-o forums, which goes a bit like...
	//Character Name
	//<i>Corporation Name</i>
	//<i>Corp Ticker</i> //eve-bb onry
	//<b>Alliance Name</b>
	//<b>Ally Ticker</b> //eve-bb onry
	
	if ($pun_config['o_eve_use_corp_name']) {
		$char_tag .= '<em class="corp_name_topic">'.$char['corp_name'].'</em><br />';
	} //End if.
	
	if ($pun_config['o_eve_use_corp_ticker']) {
		$char_tag .= '<em class="corp_ticker_topic">['.$char['ticker'].']</em><br/>';
	} //End if.
	
	if ($pun_config['o_eve_use_ally_name'] ) {
		$char_tag .= '<strong class="ally_name_topic">'.$char['ally_name'].'</strong><br />';
	} //End if.
	
	//We'll just fudge the ticker for now.
	if ($pun_config['o_eve_use_ally_ticker']) {
		$char_tag .= '<strong class="ally_ticker_topic">&lt;'.$char['shortName'].'&gt;</strong><br />';
	} //End if.
	
	if (strlen($char_tag) > 0) {
		$char_tag = '<br />'.$char_tag;
	} //End if.
	...
	if ($cur_post['poster_id'] > 1)
	{
		if ($pun_user['g_view_users'] == '1')
			$username = '<a href="profile.php?id='.$cur_post['poster_id'].'">'.$char_name.'</a>';
		else
			$username = $char_name;
		...
		if ($pun_config['o_eve_use_iga'] == '1' && $pun_config['o_avatars'] == '1') {
				$user_avatar = $char_avatar;
